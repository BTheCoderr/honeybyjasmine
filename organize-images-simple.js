...
async function main() {
  ...
  // Check if images are already in the target directory
  const existingImages = await findImagesInDirectory(TARGET_DIR);
  
  if (existingImages.length > 0) {
    console.log(`✅ Found ${existingImages.length} image(s) already in the meals directory:\n`);
    ...
    console.log('\n🔧 Let\'s match your existing images to the meals:\n');
    ...
    for (const [filename, mealName] of Object.entries(MEAL_MAPPINGS)) {
      ...
      const answer = await question(`\n   Enter the number of the image to rename (or 's' to skip): `);
      ...
      if (imageIndex >= 0 && imageIndex < existingImages.length) {
        const sourceImage = existingImages[imageIndex];
        ...
        try {
          fs.renameSync(sourceImage, finalTargetPath);
    ...
    // No images in target, ask for source
    ...
    for (const [filename, mealName] of Object.entries(MEAL_MAPPINGS)) {
      ...
      const answer = await question(`\n   Enter the number of the image to copy (or 's' to skip): `);
